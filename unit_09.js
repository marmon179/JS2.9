...
function f5() {
    ...
    if (out.classList.value === "out-4 bg-4") {
        out1.textContent = "true"
    } else {
        out1.textContent = "false"
    }
    console.log(out.classList.value)
}
...
for (let i = 0; i < div10.length; i++) {
    div10[i].onclick = () => {
        div10[i].classList.toggle("bg-10")
    }
}
...
function f18() {
    ...
    const p = document.querySelector(".p-18").getAttribute("data-b")
    out.textContent = p
}
...
function f19() {
    ...
    const p = document.querySelectorAll(".p-19[data-b]")
    let str = ""
    for (let i = 0; i < p.length; i++) {
